import type { Dict, StateMachine as S, VoidFunction, Writable } from './types'
import { cast, clear, invariant, isArray, isObject, isString, noop, runIfFn, uuid, warn } from '@destyler/utils'
...
import { ActionTypes, MachineStatus, MachineType } from './types'
...
> {
  ...
  public sendChild = (evt: S.Event<S.AnyEventObject>, to: string | ((ctx: TContext) => string)) => {
    ...
    if (!child) {
      invariant(`[@destyler/core] Cannot send '${event.type}' event to unknown child`)
    }
    child!.send(event)
  }
  ...
  public stopChild = (id: string) => {
    if (!this.children.has(id)) {
      invariant(`[@destyler/core > stop-child] Cannot stop unknown child ${id}`)
    }
  ...
  public setContext = (context: Partial<Writable<TContext>>) => {
    for (const key in context) {
      this.state.context[<keyof TContext>key] = (context as any)[key]!
    }
  }
  ...
  private executeActions = (actions: S.Actions<TContext, TState, TEvent> | undefined, event: TEvent) => {
    ...
    for (const action of toArray(_actions)) {
      ...
      warn(
        isString(action) && !fn,
        `[@destyler/core > execute-actions] No implementation found for action: \`${action}\``,
      )
  ...
  private executeActivities = (
    ...
    for (const activity of activities) {
      ...
      if (!fn) {
        warn(`[@destyler/core > execute-activity] No implementation found for activity: \`${activity}\``)
        continue
      }
  ...
  public sendParent = (evt: S.Event<S.AnyEventObject>) => {
    if (!this.parent) {
      invariant('[@destyler/core > send-parent] Cannot send event to an unknown parent')
    }
  ...
  public transition = (state: TState['value'] | S.StateInfo<TContext, TState, TEvent> | null, evt: S.Event<TEvent>) => {
    ...
    if (!stateNode && !this.config.on) {
      const msg
        = this.status === MachineStatus.Stopped
          ? '[@destyler/core > transition] Cannot transition a stopped machine'
          : `[@destyler/core > transition] State does not have a definition for \`state\`: ${state}, \`event\`: ${event.type}`
      warn(msg)
      return
    }

    const transitionConfig: S.Transitions<TContext, TState, TEvent>
      = (stateNode?.on as any)?.[event.type] ?? (this.config.on as any)?.[event.type]
...
export function createMachine<
  ...
  TEvent extends S.EventObject = S.AnyEventObject,
>(config: S.MachineConfig<TContext, TState, TEvent>, options?: S.MachineOptions<TContext, TState, TEvent>) {
