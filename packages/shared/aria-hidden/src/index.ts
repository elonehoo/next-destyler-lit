// @see https://github.com/adobe/react-spectrum/blob/main/packages/@react-aria/overlays/src/ariaHideOutside.ts
...
export function ariaHidden(targets: Array<HTMLElement | null>, rootEl?: HTMLElement) {
  ...
  const hide = (node: Element) => {
    const refCount = elementCountMap.get(node) ?? 0
  ...
  return () => {
    ...
    for (const node of hiddenNodes) {
      const count = elementCountMap.get(node)
