...
} from '@destyler/interact-outside'
import type { Layer } from './layer-stack'
import { contains, getEventTarget } from '@destyler/dom'
import {
  trackInteractOutside,
} from '@destyler/interact-outside'
import { warn } from '@destyler/utils'
...
export function trackDismissableElement(node: HTMLElement | null, options: DismissableElementOptions) {
  if (!node) {
    warn('[@destyler/dismissable] node is `null` or `undefined`')
    return
  }
  ...
  function onPointerDownOutside(event: PointerDownOutsideEvent) {
    ...
    if (event.defaultPrevented)
      return
    if (debug) {
      consola.log('onPointerDownOutside:', event.detail.originalEvent)
    }
    onDismiss?.()
  }
  ...
  function onFocusOutside(event: FocusOutsideEvent) {
    ...
    if (event.defaultPrevented)
      return
    if (debug) {
      consola.log('onFocusOutside:', event.detail.originalEvent)
    }
    onDismiss?.()
  }
