...
import { hasProp, runIfFn } from '@destyler/utils'
...
export function extractInfo<T extends AnyPointerEvent = AnyPointerEvent>(event: T, type: 'page' | 'client' = 'page') {
  ...
  return {
    point: {
      x: (point as any)[`${type}X`],
      y: (point as any)[`${type}Y`],
    },
  }
}
...
export function getEventName(evt: keyof EventMap): keyof EventMap {
  ...
  if (supportsTouchEvent())
    return (touchEventNames as any)[evt]
  if (supportsMouseEvent())
    return (mouseEventNames as any)[evt]
  return evt
}
