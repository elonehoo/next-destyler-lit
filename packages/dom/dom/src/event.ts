import type { JSX } from '@destyler/types'
import { hasProp, isObject } from '@destyler/utils'
...
export function isModifiedEvent(v: Pick<KeyboardEvent, 'ctrlKey' | 'metaKey' | 'altKey'>) {
  return v.ctrlKey || v.altKey || v.metaKey
}

export function isCtrlKey(v: Pick<KeyboardEvent, 'ctrlKey' | 'metaKey'>) {
  return isMac() ? v.metaKey && !v.ctrlKey : v.ctrlKey && !v.metaKey
}
